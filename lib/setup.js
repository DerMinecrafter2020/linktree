...
async function performSetup(config) {
  ...
  // Umgebungsvariablen fuer diesen Prozess aktualisieren
  process.env.DATABASE_URL = config.databaseUrl;
  process.env.SESSION_SECRET = config.sessionSecret || generateSecret();
  process.env.NAVIDROME_ENCRYPTION_KEY = config.encryptionKey || generateSecret();
  process.env.ADMIN_EMAIL = config.adminEmail;
  process.env.ADMIN_PASSWORD = config.adminPassword;
  process.env.NAVIDROME_URL = config.navidromeUrl || '';
  process.env.NAVIDROME_USERNAME = config.navidromeUsername || '';
  process.env.NAVIDROME_PASSWORD = config.navidromePassword || '';
  process.env.NAVIDROME_POLL_INTERVAL_SEC = String(config.navidromePollIntervalSec || '30');
