...
async function getNowPlaying() {
  ...
  const isPaused = (entry.playerState || '').toString().toLowerCase() === 'paused';
  const minutesAgo = parseFloat(entry.minutesAgo) || 0;
  const playerPosition = parseFloat(entry.playerPosition) || 0;
  // Navidrome liefert playerPosition + minutesAgo: während des Abspielens ist die
  // aktuelle Position die zuletzt gemeldete Position plus die vergangenen Minuten.
  const position = isPaused
    ? playerPosition
    : Math.min(playerPosition + minutesAgo * 60, entry.duration || Infinity);
