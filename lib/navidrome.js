...
function isRadioEntry(entry, radioStations) {
  if (!entry || !radioStations || !radioStations.length) return null;

  // Direkter Match über Sender-ID oder exakter Titel
  const exactMatch = radioStations.find(s =>
    String(s.id) === String(entry.id) ||
    (s.name && entry.title && s.name.toLowerCase() === entry.title.toLowerCase())
  );
  if (exactMatch) return exactMatch;

  // Navidrome markiert Radio-Einträge manchmal mit isVideo=true oder streamUrl
  if (entry.streamUrl || entry.homepageUrl || entry.isVideo === true) {
    const streamMatch = radioStations.find(s =>
  ...
  // (daher zusätzlich Titel-Prüfung).
  const noAlbum = !entry.albumId && !entry.album;
  ...
  if (noAlbum && noTrack) {
    const titleMatch = radioStations.find(s =>
      s.name && entry.title && entry.title.toLowerCase().includes(s.name.toLowerCase())
    );
    if (titleMatch) return titleMatch;
    // Wenn keine Station bekannt ist, aber die Heuristik stark auf Radio hindeutet,
    // als generischen Radio-Eintrag behandeln, damit der Player im Radio-Modus angezeigt wird.
    if (shortOrNoDuration) {
      return { name: entry.title || 'Radiosender', homepageUrl: null, streamUrl: null };
    }
  }
  return null;
}
