...
function isRadioEntry(entry, radioStations) {
  ...
  // Pruefen ob die Entry-ID zu einem Radiosender gehoert
  ...
  // Heuristik: Kein albumId, kein track-Nummer, keine duration → wahrscheinlich Radio
  if (!entry.albumId && !entry.track && (!entry.duration || entry.duration === 0)) {
    // Nochmal nach Titel suchen
